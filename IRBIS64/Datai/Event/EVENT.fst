...
30 0 if v997:'Z' or v972^a:'???' or v900^c:'P' or v900^c:'D' then else if v30^b.6 = s(&uf('30'),&uf('31')) then &uf('+7W1#'),&uf('+7W1#'&uf('Kgmd.mnu\'v30^b*4.2)/),&uf('+7W2#'),&uf('+7W2#'(&uf('1*R,?g11#1')/)),&uf('+7W3#'(if p(g2) then &uf('Av30^b.6#1'),g2 fi/)),&uf('+7W4#'),&uf('+7W4#'if val(v30^a)>val(g998) then v30^a else g998 fi),&uf('+7W5#'),(if g3<>'' then if val(g3)>=val(&uf('Ag4#1')) and val(g3)<=val(&uf('Av30^b#1')) then &uf('+7U5#'g3/) fi fi),(if g5<>'' then |DTM=|g5 fi/)/if g5<>'' then 'VTM=Всего в текущем месяце' fi fi fi/
30 0 if v997:'Z' then 'DZ='v30^b,if v30^b='' then v30^a fi else if v30^a<>'' then if v30^b='' and (not(v900^c:'P')) or v30^b<>'' and val(v30^b)< val(g998) then 'DZ='v30^b,if v30^b='' then v30^a fi fi fi fi/
...
972 0 if v997:'Z' then "TZ="d972,&uf('9'v972^a," ("v972^b")")  else if v30^a<>'' then if v30^b='' and (not(v900^c:'P')) or v30^b<>'' and val(v30^b)< val(g998) then "TZ="d972,&uf('9'v972^a," ("v972^b")") fi fi fi/
972 0 "TO="d972,&uf("9"v972^a," ("v972^b")")
...
200 0 if v920:'Plan' then "PL="v200^a," "v200^b, if a(v200^b)then " "v200^c fi/
